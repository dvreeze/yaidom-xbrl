...
class BulkNlFrisTestUsingIndexedElem extends Suite {

  private val pathToParentDir: java.io.File =
    (new java.io.File(classOf[BulkNlFrisTestUsingIndexedElem].getResource("kvk-rpt-grote-rechtspersoon-geconsolideerd-model-b-e-indirect-2013.xbrl").toURI)).getParentFile

  private val clazz = classOf[BulkNlFrisTestUsingIndexedElem]

  private val parSize: Int = System.getProperty("parSize", "1000").toInt
  ...
  @Test def testXbrlProcessingUsingIndexedElem(): Unit = {
    ...
    (0 to parSize).toVector.par foreach { i =>
      ...
      println(s"[testXbrlProcessingUsingIndexedElem] Iteration $i (${ms - startMs} ms)")

      val xbrlInstanceDoc = getXbrlInstanceDocumentUsingIndexedElem()

      ms = System.currentTimeMillis
      println(s"[testXbrlProcessingUsingIndexedElem] Iteration $i --> Running the tests (${ms - startMs} ms)")

      testXbrlProcessing(xbrlInstanceDoc)

      ms = System.currentTimeMillis
      println(s"[testXbrlProcessingUsingIndexedElem] Iteration $i --> Ready running the tests (${ms - startMs} ms)")
    }
  }
  ...
  private def getXbrlInstanceDocumentUsingIndexedElem(): XbrlInstanceDocument = {
