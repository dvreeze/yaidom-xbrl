...
class BulkNlFrisTestUsingSaxonElem extends Suite {

  private val pathToParentDir: java.io.File =
    (new java.io.File(classOf[BulkNlFrisTestUsingSaxonElem].getResource("kvk-rpt-grote-rechtspersoon-geconsolideerd-model-b-e-indirect-2013.xbrl").toURI)).getParentFile

  private val clazz = classOf[BulkNlFrisTestUsingSaxonElem]
  ...
  @Test def testXbrlProcessingUsingSaxonElem(): Unit = {
    ...
    (0 to parSize).toVector.par foreach { i =>
      ...
      println(s"[testXbrlProcessingUsingSaxonElem] Iteration $i (${ms - startMs} ms)")

      val xbrlInstanceDoc = getXbrlInstanceDocumentUsingSaxonElem()

      ms = System.currentTimeMillis
      println(s"[testXbrlProcessingUsingSaxonElem] Iteration $i --> Running the tests (${ms - startMs} ms)")

      testXbrlProcessing(xbrlInstanceDoc)

      ms = System.currentTimeMillis
      println(s"[testXbrlProcessingUsingSaxonElem] Iteration $i --> Ready running the tests (${ms - startMs} ms)")
    }
  }
  ...
  private def getXbrlInstanceDocumentUsingSaxonElem(): XbrlInstanceDocument = {
